...
namespace mist {
    ...
    bool Stream::saveToFile(const MistString& file) {
        FileStream output;
    ...
    StreamPtr Stream::readIntoMemory() {
        if(isValid()) {
            ...
            if(readSize != size()) {
                // log error
                log_error("mist::Stream::readIntoMemory: error happened when trying to read");
            }
    ...
    bool FileStreamWin32::open(const MistString& filename, bool canwrite, bool append, bool nocache) {
        if(file)
            return false;
    ...
    */
    ...
    bool FileStream::open(const MistString& filename, bool canwrite, bool append, bool nocache) {
        if(file != NULL)
            return false;
...
namespace mist {
    ...
    /* Client constructor */
    NetStream::NetStream(const MistString& hostName, uint16 portNum):
    iostream(NULL), 
