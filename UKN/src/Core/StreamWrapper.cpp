...
namespace ukn {
    ...
    void StreamWriter::writeLine(const String& str) {
        this->write(str);
        this->writeLineSeparator();
    }
    ...
    void TextStreamWriter::write(const String& str) {
        if(mStream && mStream->isValid()) 
    ...
    String BinaryStreamReader::readString() {
        if(mStream && mStream->isValid()) { 
            ukn_string buffer;
            uint8 c;
            mStream->read(&c, 1);
    ...
    String TextStreamReader::readString() {
        ukn_string result;
