...
namespace ukn {
	...
	bool D3D11Texture2D::load(const ResourcePtr& resource, bool createMipmap) {
        ...
        if(memStream) {
			...
			if(FAILED(D3DX11CreateShaderResourceViewFromMemory(mDevice->getD3DDevice(),
															   memData->data(),
															   memData->size(),
															   0,
															   0,
	...
	SharedPtr<uint8> D3D11Texture2D::readTextureData(uint8 level) {
	...
	void D3D11Texture2D::updateTextureData(void* data, int32 x, int32 y, uint32 width, uint32 height, uint8 level) {
        if(mTexture) {
#if defined(UKN_D3D10)	
		    D3D11_MAPPED_TEXTURE2D mappedTex;

			if(FAILED(mTexture->Map(D3D11CalcSubresource(0, 0, level),
									D3D11_MAP_WRITE_DISCARD,
									0,
									&mappedTex))) {
				log_error("error mapping texture2d data");
