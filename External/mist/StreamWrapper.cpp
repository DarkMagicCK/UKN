...
namespace mist {
    ...
    void StreamWriter::writeLine(const MistString& str) {
        this->write(str);
        this->writeLineSeparator();
    }
    ...
    mStream(stream) {
        mist_assert(stream && stream->canWrite());
    }
    ...
    void BinaryStreamWriter::write(float val) {
        this->write((uint8*)&val, sizeof(float));
    }
    
    void BinaryStreamWriter::write(double val) {
        this->write((uint8*)&val, sizeof(double));
    }
    ...
    void BinaryStreamWriter::write(const uint8* bytes, size_t start, size_t end) {
        this->write(bytes + start, end - start);
    }
    
    void BinaryStreamWriter::write(const MistString& str) {
        this->write((uint8*)str.c_str(), str.length()+1);
    }
    ...
    MistString TextStreamReader::readLine() {
        return readTill('\n');
    }
    
    MistString TextStreamReader::readTill(char end) {
        MistString result;
    ...
    MistString TextStreamReader::readNumber() {
        MistString result;
