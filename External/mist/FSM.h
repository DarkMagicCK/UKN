...
namespace mist {
    ...
    inline const state_type& simple_fsm_get_null_state();
    ...
    template<>
    inline const MistString& simple_fsm_get_null_state() {
        static MistString null(L"null");
        return null;
    }
    ...
    inline void fsm_do_nothing(FSMState&) {
    ...
    public:
        ...
        FSMManager& defState(FSMState* state, const MistString& name)  {
        ...
        void delState(const MistString& name) {
        ...
        bool defTrans(const MistString& state1, const EventType& event, const MistString& state2) {
        ...
        void delTrans(const MistString& state, const EventType& event) {
        ...
        void switchToState(const MistString& name) {
            FSMStateMap::iterator itState = this->mStates.find(name);
            if(itState != this->mStates.end()) {
        ...
        FSMState* operator[](const MistString& name) const {
    ...
    private:
        ...
        typedef std::map<MistString, FSMState*> FSMStateMap;
        FSMStateMap mStates;
